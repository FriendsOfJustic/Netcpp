...
class TcpClient : public std::enable_shared_from_this<TcpClient> {
 public:
  ...
  void AsyncConnect(const std::string &ip, uint16_t port, const std::function<void(TcpClientPtr)> &func);
  void Close();
  bool IsConnected() { return socket_->is_open(); }
  void Disconnect();
  void Send(const std::string &data);
  void AsyncSend(const std::string &data, const std::function<void(TcpClientPtr)> &func);
