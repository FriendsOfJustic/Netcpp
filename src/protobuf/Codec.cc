...
namespace NETCPP {
    ...
    uint32_t crc32(const uint8_t *data, size_t length) {
        ...
        return crc ^ 0xFFFFFFFF; // 取反得到最终结果
    }
    ...
    bool Codec::deSerialize(Buffer &buffer, BaseMessagePtr &msg) {
        auto &message = msg->message;
        // 如果不满4字节直接返回
        if (buffer.size() < Codec::FieldLenSize) {
            return false;
        }
        uint32_t msg_len = buffer.peekUInt32();
        if (msg_len <= 0 || msg_len + Codec::FieldLenSize < buffer.size()) {
            spdlog::error("invalid message length: {}", msg_len);
            return false;
        }
        buffer.discard(Codec::FieldLenSize);
        // 1. 从缓冲区读取请求ID（16字节）
        auto reqIdLen = buffer.retrieveUInt32();
        msg->NETCPP_ID = buffer.retrieveString(reqIdLen);
        // 如果够一个消息的长度
        auto typeNameLen = buffer.retrieveUInt32();
        // 1. 从缓冲区读取消息体（跳过长度字段）
        std::string typeName = buffer.retrieveString(typeNameLen);
        message = createMessage(typeName);
        if (!message) {
            spdlog::error("create message failed: {}", typeName);
            return false;
        }
        auto binaryLen = msg_len - Codec::FieldReqIdLenSize - reqIdLen - Codec::FieldTypeNameLenSize - typeName.size() -
                         Codec::FieldCheckSumLenSize;
        // 2. 解析消息体字符串到指定的消息对象（通过引用传递）
        if (!message->ParseFromArray(buffer.getReadPos(), binaryLen)) {
            spdlog::error("parse message failed");
            return false;
        }
        auto crc = crc32((uint8_t *) buffer.getReadPos(), binaryLen);
        buffer.discard(binaryLen);
        // 3. 校验和校验
        uint32_t checkSum = buffer.retrieveUInt32();
        if (checkSum != crc) {
            spdlog::error("checksum error: {} != {}", checkSum, crc);
            return false;
        }
        spdlog::debug("deSerialize {} {}", typeName, msg_len);
        return true;
    }

    void Codec::serialize(BaseMessagePtr msg, Buffer &buffer) {
        auto &message = msg->message;
        int32_t total_size = 0;
        auto typeName = message->GetDescriptor()->full_name();
        int32_t typeNameSize = typeName.size();
        auto binary = message->SerializeAsString();
        auto crc = crc32((uint8_t *) binary.data(), binary.size());
        auto &reqId = msg->NETCPP_ID;
        ...
        buffer.writeUInt32(typeNameSize);
    ...
    void Codec::serialize(BaseMessagePtr msg, std::string &buffer) {
        auto &message = msg->message;
        int32_t total_size = 0;
        auto typeName = message->GetDescriptor()->full_name();
        int32_t typeNameSize = typeName.size();
        ...
        total_size = Buffer::HostToNetInt32(total_size);
        auto reqIdLen = Buffer::HostToNetInt32(reqId.size());
        auto typeNameLen = Buffer::HostToNetInt32(typeName.size());
        buffer.append((char *) &total_size, sizeof(total_size));
        buffer.append((char *) &reqIdLen, sizeof(reqIdLen));
        buffer.append(reqId.c_str(), reqId.size());
        buffer.append((char *) &typeNameLen, sizeof(typeNameLen));
        buffer.append(typeName.c_str(), typeName.size());
        buffer.append(binary.c_str(), binary.size());
        crc = Buffer::HostToNetInt32(crc);
